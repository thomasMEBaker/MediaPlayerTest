...
{
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "Constructor");
	if (FAndroidMisc::ShouldUseVulkan()) {
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "ShouldUseVulkan = True");
	}
	else {
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "ShouldUseVulkan = False");
	}
...
FAndroidMediaPlayer::~FAndroidMediaPlayer()
{
	...
	if (JavaMediaPlayer.IsValid())
	{
		...
		{
			GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "Vulkan = FALSE");
...
//					Params.VideoTexture->Release();
		...
		{
			GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "Vulkan = TRUE");
			JavaMediaPlayer->SetVideoTexture(nullptr);
			JavaMediaPlayer->Reset();
			JavaMediaPlayer->Release();
		}
...
void FAndroidMediaPlayer::TickFetch(FTimespan DeltaTime, FTimespan /*Timecode*/)
{
	...
	if (FAndroidMisc::ShouldUseVulkan())
	{
		//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "create new video sample - 1"); //----------------------HERE-------------------------
		...
		ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				...
				const int32 CurrentFramePosition = PinnedJavaMediaPlayer->GetCurrentPosition();
				const FTimespan Time = FTimespan::FromMilliseconds(CurrentFramePosition);
				//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "create new video sample - HERE"); //----------------------HERE-------------------------
				...
				if (!PinnedJavaMediaPlayer->GetVideoLastFrameData(Buffer, BufferCount))
				{
					//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "create new video sample - 1A");
					return;
				}
								...
								if (BufferCount != Params.SampleCount)
				{
					...
					GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "AndroidMediaPlayer::Fetch: Sample count mismatch(Buffer = % d, Available = % d"); 
				}
				...
				GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "InitializeBuffer");

				PinnedSamples->AddVideo(Params.VideoSample); // removing this stops the issue but no image is rendered - expected 
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "create new video sample -2");
		struct FWriteVideoSampleParams
		{
			TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> JavaMediaPlayerPtr;
			FGuid PlayerGuid;
	...
	{
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "create new video sample-3");
		// create new video sample
		const FJavaAndroidMediaPlayer::FVideoTrack VideoTrack = VideoTracks[SelectedVideoTrack];
		auto VideoSample = VideoSamplePool->AcquireShared();
	...
	// create new video sample
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, "create new video sample - 4");
	const FJavaAndroidMediaPlayer::FVideoTrack VideoTrack = VideoTracks[SelectedVideoTrack];
	auto VideoSample = VideoSamplePool->AcquireShared();
